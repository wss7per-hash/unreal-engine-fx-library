...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        brand = QHBoxLayout()
        brand.setSpacing(10)
        # FX monogram (Stripe-style brand mark with soft gradient bg)
        fx_mono = QLabel("FX")
        fx_mono.setObjectName("heromono")
        fx_mono.setFixedSize(32, 32)
        fx_mono.setAlignment(Qt.AlignCenter)
        logo = QLabel()
        logo.setPixmap(app_icon().pixmap(28, 28))
        logo.setFixedSize(28, 28)
        brand_title = QVBoxLayout()
        brand_title.setSpacing(0)
        brand_title.setContentsMargins(0, 0, 0, 0)
        bt = QLabel(tr("app_title"))
        bt.setObjectName("brand")
        bt2 = QLabel(tr("tagline"))
        bt2.setObjectName("tagline")
        brand_title.addWidget(bt)
        brand_title.addWidget(bt2)
        brand.addWidget(fx_mono)
